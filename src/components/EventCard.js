    ...
    return (
        <div className="event-card w-full md:w-2/5 xl:1/3 m-4 h-50 rounded overflow-hidden shadow-lg hover:shadow-xl cursor-pointer">
            <div className="image-container relative">
                {(date || time) && <Tag text={`${date} ${time}`} className={"absolute float-right top-1 right-1"} />}
                {image ? <img className="w-full h-48 md:h-56 object-cover" src={image} alt={title}></img> : <div className="h-48 md:h-56"></div>}
            </div>
